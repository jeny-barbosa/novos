...
if (!empty($_POST)) {
    ...
    if ($_POST['codigo'] != '') {
        $query = "UPDATE pedidos SET nome='$nome', autor='$autor', edicao = '$edicao'WHERE codigo='" . $_POST['codigo'] . "'";
        $message = 'Dados Atualizados';
    } else {
        $query = "  
           INSERT INTO pedidos(nome, autor, edicao) VALUES('$nome', '$autor', '$edicao');";
        $message = 'Dados Inseridos';
    }
    if (mysqli_query($conn, $query)) {
        $output .= '<label class="text-success">' . $message . '</label>';
        $select_query = "SELECT * FROM pedidos ORDER BY codigo DESC";
